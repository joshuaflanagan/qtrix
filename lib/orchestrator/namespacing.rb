...
module Orchestrator
  ...
  #
  #   def some_method
  #     redis.keys  # constrained to :orchestrator:foo:*
  #   end
  # end
  module Namespacing
    ...
    # a root (orchestrator:) namespaced client.  Examples:
    ...
    # be transparen when mixing in the Orchestrator::Namespacing module.
    class Manager
      include Singleton
      ...
      def redis(*namespaces)
        namespaced_client = Redis::Namespace.new(:orchestrator, redis: client)
      ...
      def remove_namespace!(namespace)
        ...
        Orchestrator::Override.clear!(namespace)
        Orchestrator::Queue.clear!(namespace)
        Orchestrator::Matrix.clear!(namespace)
      end
      ...
      def not_ready?(namespace)
        namespace != :default &&
        Orchestrator.desired_distribution(namespace).empty?
      end
