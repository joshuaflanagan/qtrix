...
Usage: bundle exec orchestrate queues [options]
...
Options include:
      ...
      def exec_behavior
        if config[:desired_distribution]
          msg = "Queues:\n"
          msg += Orchestrator.desired_distribution.map(&stringify).join("\n")
          write(msg)
        elsif queue_weights
      ...
      def map_queue_weights(weight_map)
        Orchestrator.map_queue_weights *to_args(config_set, weight_map)
        write("OK")
      end
