...
require 'orchestrator/namespacing'
require 'orchestrator/queue'
require 'orchestrator/override'
require 'orchestrator/matrix'
...
# lib/orchestrator/matrix.rb comments.
#
...
module Orchestrator
  ...
  def self.map_queue_weights(*args)
    ...
    Orchestrator::Queue.map_queue_weights(config_set, map)
  end
  ...
  def self.add_override(*args)
    ...
    Orchestrator::Override.add(config_set, queues, processes)
    true
  end
  ...
  def self.remove_override(*args)
    config_set, queues, processes = extract_args(2, *args)
    Orchestrator::Override.remove(config_set, queues, processes)
    true
  end
  ...
  def self.overrides(config_set=:current)
    Orchestrator::Override.all(config_set)
  end
  ...
  def self.queues_for!(hostname, workers)
    overrides_queues = Orchestrator::Override.overrides_for(hostname, workers)
