...
describe Orchestrator::Queue do
  include Orchestrator::Namespacing
  before(:each) do
    Orchestrator::Queue.map_queue_weights a: 2, b: 3
  end
  let(:all_queues) {Orchestrator::Queue.all_queues}
  ...
  context "comparing two queues with the same name and weights" do
    let(:other_queue) {Orchestrator::Queue.new(:current, :a, 2)}
    describe "#==" do
  ...
  context "comparing two queues with the same name and different weights" do
    let(:other_queue) {other_queue = Orchestrator::Queue.new(:current, :a, 3)}
    describe "#==" do
  ...
  context "comparing two queues with the same weight and different names" do
    let(:other_queue) {other_queue = Orchestrator::Queue.new(:default, :b, 2)}
    describe "#==" do
      it "should return false" do
        queue1.should_not == other_queue
      end
  ...
  context "class instance methods" do
    ...
    describe "#map_queue_weights" do
      context "no namespace specified" do
        it "should remove old queues" do
          Orchestrator::Queue.map_queue_weights A: 100
          result = Orchestrator::Queue.all_queues.map(&:name)
        ...
        it "should add new queues ordered according to weight" do
          results = Orchestrator::Queue.all_queues
          results.map(&:name).should == [:A, :B, :C]
          results.map(&:weight).should == [3, 2, 1]
        end

        it "should error when trying to save empty queue name" do
          expect{Orchestrator::Queue.map_queue_weights('' => 1)}.to raise_error
        end

        it "should error when trying to save nil weight" do
          expect{Orchestrator::Queue.map_queue_weights(c: nil)}.to raise_error
        end

        it "should error when trying to save weight of 0" do
          expect{Orchestrator::Queue.map_queue_weights(c: 0)}.to raise_error
        end
      ...
      context "namespace specified" do
        it "should affect targetted namespace" do
          Orchestrator::Queue.map_queue_weights :night, Z: 10, X: 1, Y: 5
          Orchestrator::Queue.all_queues(:night).map(&:name).should == [:Z, :Y, :X]
        end
      end
    end
    ...
    describe "#all_queues" do
      describe "with no namepsace specified" do
        it "should contain all queues in the default ns sorted by weight desc" do
          Orchestrator::Queue.all_queues.map(&:name).should == [:A, :B, :C]
        end
      end

      describe "with namespace specified" do
        it "should contain all queues in the default ns sorted by weight desc" do
          result = Orchestrator::Queue.all_queues(:night)
    ...
    describe "#count" do
      context "with no namespace specified" do
        it "should be the number of queues mapped in the current namespace" do
          Orchestrator::Queue.count.should == 3
        end
      end
      context "with namespace specifeid" do
        it "should be the number of queues mapped in the target namespace" do
          Orchestrator::Queue.map_queue_weights(:night, a: 2)
          Orchestrator::Queue.count(:night).should == 1
        end
      end
    end
    ...
    describe "#clear!" do
      context "with no namespace specified" do
        before(:each) {Orchestrator::Queue.clear!}
        it "should remove all existing queue weights" do
          redis.exists(Orchestrator::Queue::REDIS_KEY).should_not == true
        end
      ...
      context "with namespace specified" do
        before(:each) {Orchestrator::Queue.clear! :night}
        it "should remove all existing queue weights" do
          redis(:night).exists(Orchestrator::Queue::REDIS_KEY).should_not == true
        end
