...
describe Orchestrator::Namespacing do
  ...
  shared_examples_for "@redis_namespace definers #redis" do
    ...
    it "should allow caller to specify namespacing" do
      ...
      result.should == ["orchestrator:default:a:b:c:d"]
    end
  end
      ...
      describe "#redis" do
        context "without @redis_namespace defined in self or self.class" do
          ...
          it "should allow caller to specify namespacing" do
            ...
            raw_redis.keys.include?("orchestrator:a:b:c").should == true
          end
          ...
          it "should evaluate caller arg of :current to the current namespace" do
            ...
            result.should == ["orchestrator:default:a"]
          end
        end
  ...
  describe Orchestrator::Namespacing::Manager do
    let(:manager) {Orchestrator::Namespacing::Manager.instance}
    ...
    describe "#redis" do
      context "with no args passed" do
        ...
        it "should return a redis connection namespaced to orchestrator:default" do
      ...
      context "with args passed" do
        it "each arg should be a namespace for keys defined in redis" do
          ...
          raw_redis.keys.include?("orchestrator:foo:bar:a").should == true
        end

        it "should prune out any duplicate namespaces" do
          manager.redis(:a, :a, :a).set("b", 1)
          result = raw_redis.keys.grep(/orchestrator:a/)
          result.should == ["orchestrator:a:b"]
        end
      end
    end
    ...
    describe "#remove_namesapce" do
      ...
      it "should not allow you to remove the current namespace" do
        Orchestrator.map_queue_weights(:transition_flood, A: 1)
      ...
      describe "cascading removal" do
        before do
          Orchestrator.add_override(:transition_flood, ["A"], 1)
          Orchestrator.map_queue_weights :transition_flood, B: 10
          Orchestrator::Matrix.queues_for!(:transition_flood, "host1", 2)
        end

        it "should cascade deletes to data in the namespace" do
          manager.remove_namespace!(:transition_flood)
          raw_redis.keys("orchestrator:transition_flood*").should == []
        end
      end
    end
    ...
    describe "#change_current_namespace" do
      ...
      it "should set the current namespace" do
        Orchestrator.map_queue_weights(:night_distribution, A: 1)
